...
import KpaVersionViewModal from './KpaVersionViewModal.jsx';

// Compact version list rendered inside the "Version history" card below
// every KPA-kind process on the whiteboard. Mirrors SowVersionHistory but
// without the signed-copy upload — KPAs don't produce a PDF artefact, so
// each row is just version + when/by + click-to-view.

export default function KpaVersionHistory({ processId, refreshKey = 0 }) {
  ...
  if (err) {
    const missing = /invalid object name|kpaversions/i.test(err);
    return (
      <div className="sow-versions muted small">
        {missing ? (
          <>
            <div style={{ marginBottom: 6 }}>
              The <code>KpaVersions</code> table doesn't exist yet — the auto-migration didn't reach the database.
  ...
  if (versions.length === 0) {
    return <div className="sow-versions muted small">No saved versions yet — save the KPA to create the first version.</div>;
  }
            ...
            >
              <div className="sow-versions-row">
                ...
                <div className="sow-versions-meta">
                  <div className="sow-versions-when">
                    {formatWhen(v.createdAt)}
                    {v.employeeName && <span className="muted small"> · {v.employeeName}</span>}
                  </div>
                  {v.createdBy && (
