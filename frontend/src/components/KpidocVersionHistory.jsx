...
import KpidocVersionViewModal from './KpidocVersionViewModal.jsx';

// Compact version list for KPI Doc tasks. Mirrors EdpVersionHistory.
export default function KpidocVersionHistory({ processId, refreshKey = 0 }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setErr('');
    api.listKpidocVersions(processId)
  ...
  if (err) {
    const missing = /invalid object name|kpidocversions/i.test(err);
    return (
      <div className="sow-versions muted small">
        {missing ? (
          <>
            <div style={{ marginBottom: 6 }}>
              The <code>KpidocVersions</code> table doesn't exist yet — the auto-migration didn't reach the database.
            </div>
            <button
              type="button"
              className="btn-ghost"
  ...
  if (versions.length === 0) {
    return <div className="sow-versions muted small">No saved versions yet — save the KPI Document to create the first version.</div>;
  }
