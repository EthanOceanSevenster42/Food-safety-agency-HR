...
import JdVersionViewModal from './JdVersionViewModal.jsx';

// Compact version list rendered inside the "Version history" card below
// each JD-kind task on the whiteboard. Same pattern as KpaVersionHistory.
export default function JdVersionHistory({ processId, refreshKey = 0 }) {
  ...
  if (err) {
    const missing = /invalid object name|jdversions/i.test(err);
    return (
      <div className="sow-versions muted small">
        {missing ? (
          <>
            <div style={{ marginBottom: 6 }}>
              The <code>JdVersions</code> table doesn't exist yet — the auto-migration didn't reach the database.
            </div>
            <button
              type="button"
              className="btn-ghost"
  ...
  if (versions.length === 0) {
    return <div className="sow-versions muted small">No saved versions yet — save the JD to create the first version.</div>;
  }
